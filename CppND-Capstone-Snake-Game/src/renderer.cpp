...
void Renderer::Render(Snake const snake, SDL_Point const &food, std::set<TntBomb> const &tnts){
  SDL_Rect block;
  ...
  for(int i=0; i<grid_height+1; i++) {
    SDL_RenderDrawLine(sdl_renderer, 0, block.h*i, screen_width, block.h*i);
  }

  // Draw column lines
  for(int j=0; j<grid_width+1; j++) {
    SDL_RenderDrawLine(sdl_renderer, block.w*j, 0, block.w*j, screen_height);
  }
